...
export function showInfoPanel(
  ...
  document.getElementById('csv-node-details')?.remove();
  if(node.properties){
    const details=document.createElement('details');details.id='csv-node-details';
    const summary=document.createElement('summary');summary.textContent='Imported properties and relations';details.append(summary);
    const pre=document.createElement('pre');pre.style.whiteSpace='pre-wrap';pre.textContent=JSON.stringify({properties:node.properties,relations:node.csvRelations},null,2);details.append(pre);
    document.getElementById('info-long-description').after(details);
  }
  // Definition
  if(node.properties)document.getElementById("info-definition").textContent=node.definition;else renderMarkdown(document.getElementById("info-definition"), node.definition);
