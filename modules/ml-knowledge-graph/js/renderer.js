...
export function createEdges(edges) {
  ...
  for (const edge of edges) {
    ...
    if (si !== undefined && ti !== undefined) {
      edgeList.push({...createEdgeDescriptor(si, ti), ...edge});
    }
  }
...
function positionArrows(arrowMesh, list) {
  ...
  for (let i = 0; i < list.length; i++) {
    const edge = list[i];
    if(edge.directed === false){for(let a=0;a<ARROWS_PER_EDGE;a++)hideArrowInstance(arrowMesh,instanceIdx++);continue;}
...
function collectPairsForNodeSet(nodeSet) {
  // Retain typed parallel edges and undirected markers from the canonical records.
  return edgeList.filter(edge => nodeSet.has(nodes[edge.si].id) && nodeSet.has(nodes[edge.ti].id));
}
