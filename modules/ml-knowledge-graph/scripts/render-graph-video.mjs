...
async function startStaticServer(rootDir) {
  ...
  const platformPython=path.resolve(rootDir,'../../.venv',process.platform==='win32'?'Scripts/python.exe':'bin/python');
  const python=process.env.MATH_UTILS_PYTHON||(fsSync.existsSync(platformPython)?platformPython:process.platform==='win32'?'python':'python3');
  const child=spawn(python,['-B',path.join(rootDir,'app.py'),'--host','127.0.0.1','--port',String(port),'--no-browser'],{cwd:rootDir,windowsHide:true,stdio:['ignore','ignore','pipe']});
  let errors='';let launchError=null;child.stderr.on('data',data=>errors+=String(data));child.on('error',error=>launchError=error);
  const url=`http://127.0.0.1:${port}/`;
  for(let attempt=0;attempt<240;attempt++){
    if(launchError||child.exitCode!==null)throw new Error(`Local backend failed: ${launchError||errors}`);
    try{const response=await fetch(url+'api/health');if(response.ok)return {url,server:{close(callback){child.once('exit',callback);child.kill();}}};}catch{}
    await new Promise(resolve=>setTimeout(resolve,250));
  }
  child.kill();throw new Error('Local graph backend did not become ready within 60 seconds.');
}
