...
No build step is required. The integrated interface uses a local Python API for graph mathematics, CSV validation, layouts and TDA, so start its server from this directory after installing the root requirements:

```bash
python app.py --host 127.0.0.1 --port 8004 --no-browser
```

Then open http://127.0.0.1:8004. No internet connection is required at runtime.

## Layout Cache

The app reads the shipped `knowledge_graph.layout.json` as the precomputed positions for the bundled graph's default force layout.

- The readiness endpoint validates that the cache contains exactly the bundled node IDs.
- Other graphs and explicit non-bundled force calculations use the Python layout API.
...
## Math Utils integration

Run the root Math Utils launcher or python app.py --port 8004. Settings contains Import CSV / TDA. See docs/INTEGRATION.md and docs/CSV_SCHEMA.md for the local Python API, offline assets, independent edge fields, validation, and numerical compatibility notes. The static-only launch instructions above describe the original fork; the integrated runtime requires this local Python backend.
